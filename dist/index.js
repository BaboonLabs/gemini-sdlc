...
const server = new Server({ name: "gemini-sdlc", version: "3.0.0" }, { capabilities: { tools: {} } });
// =============================================================================
// TOOL DEFINITIONS
// =============================================================================
const tools = [
...
function estimateEffort(scope, complexity = "medium") {
    const multipliers = { low: 0.5, medium: 1.0, high: 2.0, very_high: 3.5 };
...
function generateDockerfile(language, framework = "", port = 3000) {
    const dockerfiles = {
...
function generateChangelog(version, changes, date = "") {
    if (!date) {
...
function analyzeGitHistory(targetPath = ".", days = 30) {
    try {
        const result = execSync(`git log --since="${days} days ago" --pretty=format:"%h|%an|%s" --shortstat`, {
            cwd: targetPath,
            encoding: "utf8",
            maxBuffer: 10 * 1024 * 1024,
            timeout: 30000
    ...
    try {
        switch (name) {
            ...
            case "generate_cloud_run_config":
                result = generateCloudRunConfig(getArg("service_name", ""), getArg("memory", "512Mi"), getArg("cpu", "1"), getArgNumber("min_instances", 0), getArgNumber("max_instances", 10));
                break;
